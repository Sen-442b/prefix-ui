...
const emailValidationSuccess=document.querySelector("#email-validation-success");
...
const passwordValidationSuccess=document.querySelector("#password-validation-success");
const passwordMatchInput=document.querySelector("#password-match-input");
const passwordMatchTextError=document.querySelector("#password-match-text-error");
const passwordMatchValidationError=document.querySelector("#password-match-validation-error");
const passwordMatchValidationSuccess=document.querySelector("#password-match-validation-success");
...
const validateEmail=email=>email.split("").filter(char=>char==="@").length===1;
const validatePassword= password => {
    const regex= new RegExp("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{8,})");
    return regex.test(password);
};
const validatePasswordConfirmation=(eventTargetValue,passwordInputValue)=>eventTargetValue===passwordInputValue;
